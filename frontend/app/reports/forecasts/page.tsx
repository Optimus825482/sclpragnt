...
const accuracyTone = (value: unknown, warn = false) => {
  ...
  return Number(value) >= 0.55 ? "text-neon-green" : warn ? "text-yellow-300" : "text-neon-red";
};
  ...
  return <main className="page-shell space-y-5">
    ...
    {report && <>
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Metric title="Yön doğruluğu" value={pct(report.directional_accuracy)} tone={accuracyTone(report.directional_accuracy, true)} />
      ...
      <section className="card"><p className="eyebrow">UFUK BAZLI BAŞARI</p><div className="mt-3 table-scroll"><table className="data-table"><thead><tr><th>Ufuk</th><th>Ölçülen</th><th>Doğru</th><th>Yön doğruluğu</th><th>Ort. güven</th><th>Ort. hareket</th><th>Bekleyen</th></tr></thead><tbody>{(report.horizons || []).map((row:any) => <tr key={row.horizon_minutes}><td>{row.horizon_minutes} dk</td><td>{row.evaluated_count || 0}</td><td>{row.correct_count || 0}</td><td className={accuracyTone(row.directional_accuracy)}>{pct(row.directional_accuracy)}</td><td>{row.average_confidence == null ? "—" : `%${Number(row.average_confidence).toFixed(0)}`}</td><td>{pct(row.average_return_pct)}</td><td>{row.pending_count || 0}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs text-bunker-muted">Güven, LLM’in beyanıdır; doğrulukla ayrı değerlendirilir. Az örneklemde sonuç karar kanıtı değildir.</p></section>
      <section className="card"><p className="eyebrow">SON YORUMLAR</p><div className="mt-3 table-scroll"><table className="data-table"><thead><tr><th>Zaman</th><th>Sembol</th><th>Ufuk</th><th>Tahmin</th><th>Güven</th><th>Sonuç</th><th>Hareket</th></tr></thead><tbody>{(report.recent || []).map((row:any) => <tr key={row.forecast_id}><td>{new Date(toMs(row.created_at)).toLocaleString("tr-TR")}</td><td><SymbolLink symbol={row.symbol} className="text-white hover:text-neon-green" /></td><td>{row.horizon_minutes} dk</td><td>{direction(row.direction)}</td><td>{row.confidence == null ? "—" : `%${Math.round(Number(row.confidence))}`}</td><td className={row.status === "evaluated" ? (row.direction_correct ? "text-neon-green" : "text-neon-red") : "text-yellow-300"}>{row.status === "evaluated" ? (row.direction_correct ? "DOĞRU" : "YANLIŞ") : "BEKLİYOR"}</td><td>{row.status === "evaluated" ? pct(row.outcome_return_pct) : "—"}</td></tr>)}</tbody></table></div></section>
    </>}
  </main>;
}
