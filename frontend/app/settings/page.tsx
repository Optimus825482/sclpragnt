          ...
          <div className={`card bg-bunker-950 ${activeTab !== "symbols" ? "hidden" : ""}`}>
            ...
            <div className="mt-5 border-t border-bunker-800 pt-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 max-h-[32rem] overflow-y-auto pr-1">
                {visibleActivity.map((item: any) => <div key={item.symbol} className={`rounded-lg border px-3 py-2 ${item.status === "ACTIVE" ? "border-neon-green/30 bg-neon-green/5" : item.status === "WARMING" ? "border-sky-400/30 bg-sky-400/5" : "border-bunker-800 bg-bunker-900/60"}`}><div className="flex items-center justify-between gap-2"><SymbolLink symbol={item.symbol} className="font-mono text-sm text-white hover:text-neon-green" /><span className={`font-mono text-[10px] ${item.status === "ACTIVE" ? "text-neon-green" : item.status === "WARMING" ? "text-sky-300" : "text-yellow-300"}`}>{item.status}</span></div><div className="mt-2 grid grid-cols-3 gap-2 text-[10px] font-mono text-bunker-muted"><span>30m {item.range_30m_pct == null ? "—" : `${item.range_30m_pct}%`}</span><span>ATR {item.atr_pct == null ? "—" : `${item.atr_pct}%`}</span><span>VOL {item.volume_ratio == null ? "—" : `${item.volume_ratio}x`}</span></div><p className="mt-2 truncate text-[10px] text-bunker-muted" title={item.reason || ""}>{item.reason || "—"}{item.spread_pct != null ? ` · spread ${item.spread_pct}%` : ""}</p></div>)}
          ...
          <div className={`card bg-bunker-950 ${activeTab !== "app" ? "hidden" : ""}`}>
            <div className="border-b border-bunker-800 pb-5 mb-5">
              ...
              <div className="grid sm:grid-cols-3 gap-3 mt-3">
                ...
                <label className="rounded-lg border border-bunker-800 bg-bunker-900 px-3 py-2"><span className="font-mono text-xs text-bunker-muted">MFI giriş üst sınırı</span><input type="number" min={1} max={99} step={1} value={num(draft.bb_mfi_entry_mfi_max)} onChange={e => setDraft(d => ({ ...d, bb_mfi_entry_mfi_max: Number(e.target.value) }))} className="mt-1 w-full bg-bunker-950 border border-bunker-700 rounded px-2 py-1.5 font-mono text-xs text-white" /></label>
                <label className="rounded-lg border border-bunker-800 bg-bunker-900 px-3 py-2"><span className="font-mono text-xs text-bunker-muted">RSI çıkış alt sınırı</span><input type="number" min={1} max={99} step={1} value={num(draft.bb_mfi_exit_rsi_min)} onChange={e => setDraft(d => ({ ...d, bb_mfi_exit_rsi_min: Number(e.target.value) }))} className="mt-1 w-full bg-bunker-950 border border-bunker-700 rounded px-2 py-1.5 font-mono text-xs text-white" /></label>
                <label className="rounded-lg border border-bunker-800 bg-bunker-900 px-3 py-2"><span className="font-mono text-xs text-bunker-muted">MFI çıkış alt sınırı</span><input type="number" min={1} max={99} step={1} value={num(draft.bb_mfi_exit_mfi_min)} onChange={e => setDraft(d => ({ ...d, bb_mfi_exit_mfi_min: Number(e.target.value) }))} className="mt-1 w-full bg-bunker-950 border border-bunker-700 rounded px-2 py-1.5 font-mono text-xs text-white" /></label>
              </div>
