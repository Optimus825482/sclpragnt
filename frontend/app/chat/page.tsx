...
type AgentEvaluation = { id: number; evaluator_type: string; score?: number; passed: boolean; failure_category?: string };
type AgentTrace = { trace_id: string; intent?: string; status: string; started_at?: string };
const TOOL_GROUPS = [
...
export default function ChatPage() {
  ...
  useEffect(() => { Promise.all([fetch(`${API_BASE}/api/llm/config`).then(r => r.json()), fetch(`${API_BASE}/api/llm/chat-settings`).then(r => r.json())]).then(([data, settings]) => { setSkills(data.skills || []); if (Array.isArray(settings.active_tools)) setActiveTools(settings.active_tools); if (Array.isArray(settings.active_skills)) setActiveSkills(settings.active_skills); chatSettingsReady.current = true; }).catch(() => undefined); }, []);
  useEffect(() => { if (!chatSettingsReady.current) return; const timer = window.setTimeout(() => { fetch(`${API_BASE}/api/llm/chat-settings`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active_tools: activeTools, active_skills: activeSkills }) }).catch(() => undefined); }, 250); return () => window.clearTimeout(timer); }, [activeTools, activeSkills]);
  ...
  useEffect(() => { const load = () => Promise.all([fetch(`${API_BASE}/api/llm/evaluations?limit=8`).then(r => r.json()), fetch(`${API_BASE}/api/llm/instincts?status=active&limit=6`).then(r => r.json()), fetch(`${API_BASE}/api/llm/agent-traces?limit=8`).then(r => r.json())]).then(([evaluationData, instinctData, traceData]) => { setEvaluations(evaluationData.evaluations || []); setInstincts(instinctData.instincts || []); setTraces(traceData.traces || []); }).catch(() => undefined); load(); const timer = window.setInterval(load, 5000); return () => window.clearInterval(timer); }, []);
  ...
  const saveChatSettings = async () => { await fetch(`${API_BASE}/api/llm/chat-settings`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active_tools: activeTools, active_skills: activeSkills }) }); setSettingsOpen(false); };
  return <div className="chat-page">
    ...
    <div className="chat-layout"><Card className="chat-conversation"><SectionHeader eyebrow="SOHBET" title="Araştırma asistanı" description="Yanıtlar Türkçe ve başlıklar, listeler, kodlar ile ayrıştırılmış olarak gösterilir." /><div className="chat-messages">{messages.map((message, index) => <div key={index} className={`chat-message ${message.role}`}><div className="chat-avatar">{message.role === "user" ? "SİZ" : "AI"}</div><div className="chat-bubble"><p className="eyebrow mb-2">{message.role === "user" ? "KULLANICI" : "SCALPER"}</p><MarkdownMessage content={message.content} /></div></div>)}{busy && <div className="chat-thinking"><span className="status-dot" /> Araçlar ve model yanıtı hazırlanıyor…</div>}<div ref={endRef} /></div><form onSubmit={send} className="chat-composer"><textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Bir soru sor… örn. Komisyon sonrası en iyi strateji hangisi?" rows={2} disabled={busy} /><Button variant="primary" type="submit" disabled={busy || !input.trim()}>GÖNDER ↗</Button></form>{error && <p className="mt-3 text-xs text-neon-red">{error}</p>}</Card>
      <Card className="chat-controls"><div className="chat-controls-header"><SectionHeader eyebrow="AKTİF DURUM" title="Model akışı" description="Kullanılan araçlar ve son çağrı kayıtları." /><Button variant="secondary" onClick={() => setSettingsOpen(true)}>⚙ AYARLAR</Button></div><div className="chat-live-tools"><p className="eyebrow mb-2">AKTİF ARAÇLAR · {activeTools.length}</p><div className="chat-tool-chips">{activeTools.map(name => <Badge key={name} tone="info">{name}</Badge>)}</div></div><div className="chat-log-panel"><div className="flex items-center justify-between mb-2"><p className="eyebrow">CANLI TOOL LOGLARI</p><span className="status-dot" /></div>{logs.length ? logs.slice(0, 12).map(log => <div className="chat-log-row" key={log.id}><div><p className="font-mono text-xs text-white truncate">{log.tool_name}</p><p className="text-[10px] text-bunker-muted">{log.result_summary || (log.success ? "başarılı" : "hata")}</p></div><span className={log.success ? "text-neon-green" : "text-neon-red"}>{log.duration_ms ? `${Math.round(log.duration_ms)}ms` : "•"}</span></div>) : <p className="text-xs text-bunker-muted">Henüz tool çağrısı yok.</p>}</div><div className="chat-log-panel"><p className="eyebrow mb-2">TRACE / EVALUATION / INSTINCT</p><div className="grid grid-cols-3 gap-2 text-[10px] font-mono"><span className="rounded border border-bunker-700 p-2">Trace: {traces.length}</span><span className="rounded border border-bunker-700 p-2">Eval: {evaluations.filter(item => item.passed).length}/{evaluations.length || 0}</span><span className="rounded border border-bunker-700 p-2">Kural: {instincts.length}</span></div>{traces.slice(0, 2).map(item => <div className="chat-log-row" key={item.trace_id}><span className="truncate">{item.intent || item.trace_id.slice(0, 18)}</span><span className={item.status === "completed" ? "text-neon-green" : "text-yellow-300"}>{item.status}</span></div>)}{evaluations.slice(0, 3).map(item => <div className="chat-log-row" key={item.id}><span>{item.evaluator_type}</span><span className={item.passed ? "text-neon-green" : "text-neon-red"}>{item.passed ? "PASS" : item.failure_category || "FAIL"}</span></div>)}</div></Card>
    </div>
