...
type ScanLog = { timestamp?: number; action?: string; candidates?: number; opened?: number; error?: string };
type Monitor = { items: Candidate[]; history: Signal[]; scan_logs?: ScanLog[]; generated_at?: number; paper_trades?: unknown[]; config?: Record<string, unknown> };
...
export function PumpMonitorPanel() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="card">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="eyebrow">OTOMATİK TARAMA DENETİMİ</p><p className="text-xs text-bunker-muted">Uygun aday mı oluşmadı, yoksa tarama mı çalışmadı; burada ayrışır.</p></div><span className="font-mono text-xs text-bunker-muted">çalışma belleği · son 80 kayıt</span></div>
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Zaman</th><th>Sonuç</th><th>Uygun aday</th><th>Açılan paper</th><th>Hata</th></tr></thead><tbody>{(data?.scan_logs || []).slice(0, 12).map((log, index) => <tr key={`${log.timestamp}-${index}`}><td>{time(log.timestamp)}</td><td className={log.action === "ERROR" ? "text-neon-red" : "text-neon-green"}>{log.action || "SCAN"}</td><td>{log.candidates ?? 0}</td><td>{log.opened ?? 0}</td><td className="max-w-80 truncate text-bunker-muted" title={log.error}>{log.error || "—"}</td></tr>)}</tbody></table></div>
      {!data?.scan_logs?.length && <p className="py-4 text-center text-sm text-yellow-300">Bu çalışma sürecinde henüz otomatik tarama kaydı yok. Arka plan döngüsü 5 dk mum kapanışında kayıt üretir.</p>}
    </section>
