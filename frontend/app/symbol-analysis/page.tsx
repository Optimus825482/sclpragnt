...
export default function SymbolAnalysisPage() {
  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    ...
    <section className="card border-neon-green/25 bg-gradient-to-br from-neon-green/10 via-bunker-950 to-bunker-950"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow text-neon-green">OLASILIKLI LLM SENARYOSU</p><p className="mt-1 text-sm text-bunker-muted">M1 · M5 · M15 · M30 · H1 · H4 · D1 ve sembolün yakın fiyat davranışıyla kısa vade senaryosu üretir.</p></div><button onClick={askCommentary} disabled={commentaryLoading} className="ui-button ui-button-primary">{commentaryLoading ? "DEĞERLENDİRİLİYOR..." : "NE OLABİLİR?"}</button></div>{commentary && <div className="mt-4 rounded-lg border border-bunker-700 bg-bunker-950/70 p-4 text-sm text-slate-200"><p className="leading-7">{commentary.error || commentary.commentary}</p>{commentary.forecasts && <div className="mt-4 grid gap-3 md:grid-cols-3">{commentary.forecasts.map((item:any) => <div key={item.forecast_id} className="rounded border border-bunker-700 bg-bunker-900/50 p-3"><p className="eyebrow">{item.horizon_minutes < 60 ? `${item.horizon_minutes} DK` : `${item.horizon_minutes / 60} SAAT`}</p><p className={item.direction === "up" ? "mt-2 font-mono text-neon-green" : item.direction === "down" ? "mt-2 font-mono text-neon-red" : "mt-2 font-mono text-yellow-300"}>{item.direction === "up" ? "YUKARI" : item.direction === "down" ? "AŞAĞI" : "YATAY"} · %{Math.round(item.confidence)}</p><p className="mt-2 text-xs leading-5 text-slate-200">{item.scenario}</p><p className="mt-2 text-xs text-bunker-muted">Bozulma: {item.invalidation_price == null ? "belirtilmedi" : `₺${value(item.invalidation_price)}`}</p></div>)}</div>}</div>}</section>
