...
export default function SymbolAnalysisPage() {
  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    <header className="space-y-4"><div className="flex flex-wrap justify-between gap-3"><div><h1 className="font-mono text-xl font-bold"><span className="text-neon-green">SEMBOL</span> ANALİZİ</h1><p className="eyebrow mt-1">{symbol} · {timeframe} · canlı public data</p></div><select value={symbol} onChange={e => setSymbol(e.target.value)} className="input max-w-40">{symbols.map(s => <option key={s}>{s}</option>)}</select></div><div className="flex max-w-full gap-1 overflow-x-auto border border-bunker-800 rounded-lg p-1 w-fit">{["1m", "5m", "15m", "30m", "1h", "4h", "1d"].map(tf => <button key={tf} onClick={() => setTimeframe(tf)} className={`min-h-10 shrink-0 px-3 rounded font-mono text-xs ${timeframe === tf ? "bg-neon-green/15 text-neon-green" : "text-bunker-muted"}`}>{tf}</button>)}</div></header>
