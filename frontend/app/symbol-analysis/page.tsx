  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    ...
    {!data?.data_ready ? <Card title="VERİ DURUMU"><p className="text-bunker-muted">{error || data?.error || "Veri hazırlanıyor..."}</p></Card> : <>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Card title="HACİM / TEKNİK GÖSTERGELER"><div className="grid grid-cols-2 gap-3">{[["Hacim", volume.volume_ratio_20 == null ? "—" : `${value(volume.volume_ratio_20)}x`], ["VWAP", value(volume.vwap)], ["ATR %", percent(volatility.atr_pct)], ["RSI", value(momentum.rsi_14)], ["MACD", value(momentum.macd?.histogram)], ["ADX", value(trend.adx?.adx)]].map(([name, val]) => <div key={name}><p className="text-xs text-bunker-muted">{name}</p><p className="font-mono text-sm">{val}</p></div>)}</div></Card>
      </div>
    </>}
  </div>;
}
