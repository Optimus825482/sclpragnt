...
export default function TradeRepairPage() {
  ...
  return <main className="page-shell space-y-5">
    ...
    {legacy.length > 0 && <div className="card border-neon-red/30 bg-neon-red/5"><p className="eyebrow text-neon-red">MİGRASYON KAYNAKLI LEGACY KAYITLAR</p><div className="mt-3 space-y-1 font-mono text-sm">{legacy.map(x => <div key={x.trade_id}>{x.trade_id} · {x.symbol} · PnL ₺{Number(x.pnl || 0).toFixed(2)}</div>)}</div><button onClick={purgeLegacy} disabled={busy} className="ui-button mt-4 border-neon-red/50 text-neon-red">ONAYLA VE LEGACY KAYITLARINI SİL</button></div>}
