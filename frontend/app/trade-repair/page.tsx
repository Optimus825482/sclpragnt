...
export default function TradeRepairPage() {
  ...
  return <main className="page-shell space-y-5">
    ...
    <div className="card"><p className="eyebrow mb-3">CANLI ONARIM LOGU</p><div className="max-h-80 overflow-auto space-y-2 font-mono text-xs">{(data?.logs||[]).map((l:any,i:number)=><div key={i} className="border-b border-bunker-800 pb-2"><span className="text-bunker-muted">{new Date(toMs(l.time)).toLocaleTimeString("tr-TR")}</span> <span className={l.level==="error"?"text-neon-red":l.level==="warning"?"text-yellow-300":"text-neon-green"}>[{l.level}]</span> {l.message}</div>)}{!data?.logs?.length&&<span className="text-bunker-muted">Henüz log yok. Önizleme ile başlayın.</span>}</div></div>
  </main>;
}
