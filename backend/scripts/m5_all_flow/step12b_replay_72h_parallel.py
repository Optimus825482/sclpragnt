...
CONCURRENCY = 10  # paralel sembol isleyicisi

# step11 coklu filtre esikleri (grup_anahtar: esik)
FILTERS = [
    ("g0", "chg5", 1.2177),   # M5 G0 5dk degisim
    ("g0", "chg3", 0.8834),   # M5 G0 3dk degisim
    ("g0", "roc", 1.6839),    # M5 G0 ROC
    ("g0", "atr_pct", 0.5779),# M5 G0 ATR%
    ("g1", "atr_pct", 0.5432),# M5 G1 ATR%
    ("g2", "atr_pct", 0.5097),# M5 G2 ATR%
]
...
def _process_symbol(sym, test_start, test_end, M5_WARMUP, M1_WARMUP):
    """Tek sembol icin tum M5 kapanislarini isle; sinyal kayitlarini dondur."""
    local = get_pg()  # thread'e ozgu baglanti
    sym_records = []
    try:
        m5_all = get_candles(local, sym, "5m", test_start - M5_WARMUP - 600000, test_end)
        if len(m5_all) < 100:
            return sym_records
        for k in range(30, len(m5_all) - 1):
            ...
            m1_all = get_candles(local, sym, "1m", cur_ms - M1_WARMUP, cur_ms - 1)
            if len(m1_all) < 60:
                continue
            ...
            def chk(v, lo, val=1.0, invert=False):
                ...
                if v is None:
                    return
                ok = v >= lo if not invert else v <= lo
                if ok:
                    if val < 0:
                        neg += 1
                    score += val
            ...
            if neg > 0 or score < MIN_SCORE:
                continue
            fut = get_candles(local, sym, "1m", cur_ms, cur_ms + 6 * 60000)
            if len(fut) < 3:
                continue
            ...
            hit = upside >= TARGET
            passes_f65 = all(
                ({"g0": g0_v, "g1": g1_v, "g2": g2_v}.get(grp, {}).get(var) is not None
                 and {"g0": g0_v, "g1": g1_v, "g2": g2_v}[grp][var] >= thr)
                for grp, var, thr in FILTERS
            )
            sym_records.append({
...
def main():
    ...
    print(f"72s paralel replica: {time.strftime('%m-%d %H:%M', time.localtime(test_start/1000))} -> "
          f"{time.strftime('%m-%d %H:%M', time.localtime(test_end/1000))} (son 6s haric, concurrency={CONCURRENCY})")
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as ex:
        futures = {ex.submit(_process_symbol, sym, test_start, test_end, M5_WARMUP, M1_WARMUP): sym for sym in symbols}
        for fut in as_completed(futures):
            done += 1
            try:
                records.extend(fut.result())
            except Exception as exc:
                print(f"  hata {futures[fut]}: {exc}")
            if done % 25 == 0:
                print(f"  {done}/{len(symbols)} sembol | {len(records)} sinyal | {time.time()-t0:.0f}s")
    print(f"Tamamlandi: {done} sembol | {time.time()-t0:.0f}s")
    total = len(records)
