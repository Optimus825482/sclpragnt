"""Paper-only dynamic top-10 gainer replay for Vortex Breakout PRO v4.1.

The universe is refreshed every 15 minutes from point-in-time 24h returns
computed on completed M5 candles. Only symbols in the active top-10 set are
eligible for new entries. The replay uses a common wallet and one open
position at a time.
"""
import argparse
import asyncio
...
from scripts.replay_vortex_breakout_pro_hmtry_m5 import (
    MS_5M,
    compute_indicators,
    cost_parameters,
    iso,
...
LOOKBACK_BARS_24H = 288


async def fetch_symbol(symbol, days, cutoff, semaphore):
    async with semaphore:
        try:
            rows = normalize(await historical_klines(symbol, "5m", days, cutoff), cutoff)
...
def build_universe_sets(symbol_rows, refreshes, refresh_ms):
    ...
    for refresh in refreshes:
        past = refresh - LOOKBACK_BARS_24H * MS_5M
        ranked = []
...
def main_result(events, window_rows, active_sets, additions, removals, errors, args, costs):
    ...
    return {
        ...
        "strategy": "Vortex Breakout PRO v4.1 - dynamic top-10 gainer universe",
        "window": {
            "start": iso(args.start_ms),
            "end": iso(args.end_ms),
            "hours": args.hours,
        },
        "source": "Binance TR public completed M5 OHLCV; rolling 24h point-in-time returns",
        "universe": {
        ...
        "execution": {
            "entry": "next completed M5 bar open",
        ...
        "limitations": [
            "Dynamic universe is reconstructed from completed public M5 candles, not the historical HTML top-gainer page.",
...
async def main(args):
    ...
    for symbol, rows, error in loaded:
        if error or len(rows) < LOOKBACK_BARS_24H + 250:
            errors[symbol] = error or f"insufficient history ({len(rows)} M5 candles)"
            continue
